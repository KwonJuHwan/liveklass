...
| :--- | :--- | :--- |
| **Amazon EC2** | **이벤트 생성 (Backend API)** | 사용자의 HTTP 요청을 받아 FSM 기반의 상태 전이 로그를 생성하는 백엔드 컴퓨팅 자원입니다. 유저와 직접 통신해야 하므로 **Public Subnet**에 배치하여 트래픽 진입점 역할을 수행합니다. |
| **Amazon MSK** | **데이터 버퍼 (Message Broker)** | EC2와 RDS 사이의 의존성을 끊어주는 비동기 완충재 역할을 합니다. 트래픽 발생 시 로그를 안전하게 보관하여 DB가 받을 부하를 제어합니다. |
| **Amazon RDS** | **데이터 적재 (Log Storage)** | 정형화된 로그 스키마를 유지하고 관리하는 핵심 저장소입니다. 보안을 위해 외부 접근이 원천 차단된 **Private Subnet**에 격리하여 배치했습니다. |
